...
fn main() -> Result<()> {
    ...
    let mappings: Vec<Mapping> = match matches.value_of("rules-file") {
        ...
        None => match mapping_from_string(&create_directives(), matches.value_of("rules-arg").unwrap()) {
...
fn mappings_from_file(all_directives: &Vec<Box<dyn MappingDirective>>, file: &PathBuf) -> Result<Vec<Mapping>> {
    let f = fs::File::open(file)
                .chain_err(|| format!("Unable to open directive file {}", file.to_string_lossy()))?;
    let mut mappings = vec![];
    for line_result in BufReader::new(f).lines() {
        let line = line_result.chain_err(|| format!("Error reading directive file {}", file.to_string_lossy()))?;
...
fn get_file_paths(directory: &PathBuf) -> Result<Vec<PathBuf>> {
    ...
    for entry in fs::read_dir(directory)
                        .chain_err(|| format!("Unable to read entries of directory {}",
                                              directory.to_string_lossy()))? {
        let dir_entry = entry.chain_err(|| format!("Unable to read entry of directory {}",
                                                   directory.to_string_lossy()))?;
